...
export function useCart() {
  ...
  const addItem = useCallback(async (item: CartItem) => {
    ...
    const current = cid ? await storage.getCartItems(cid) as CartItem[] : [];
  ...
  const removeItem = useCallback(async (productId: string, variant?: Record<string, string>) => {
  ...
  const updateQuantity = useCallback(async (productId: string, quantity: number, variant?: Record<string, string>) => {
    const cid = clientId || await storage.getCartClientId();
    const current = cid ? await storage.getCartItems(cid) as CartItem[] : [];
    const vk = variant ? JSON.stringify(variant) : '';
