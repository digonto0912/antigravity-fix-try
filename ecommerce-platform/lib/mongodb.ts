...
const connections: Map<number, ClusterConnection> = new Map();

/** Get a MongoDB connection for a given cluster index (0-based) */
export async function getMongoDb(clusterIndex: number = 0): Promise<Db> {
...
export function getClusterIndex(userId: string): number {
  let hash = 0;
