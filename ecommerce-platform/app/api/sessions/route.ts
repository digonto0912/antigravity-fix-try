...
export async function POST(req: NextRequest) {
  try {
    ...
    const { sessionId, fingerprintId, userId, events, metadata } = body as {
      sessionId: string;
    ...
    // Upsert session — append events to existing session or create new
    const updateDoc: Record<string, unknown> = {
      ...
      $push: { events: { $each: events } } as unknown,
      $set: {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (fingerprintId) {
      // Query specific cluster for this fingerprint
      const clusterIdx = getClusterIndex(fingerprintId);
      const db = await getMongoDb(clusterIdx);
      ...
      // Query ALL clusters for general listing
