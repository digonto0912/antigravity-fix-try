...
import type { Order, DeliveryTracking } from '@/lib/types';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div><label className="block text-sm text-gray-600 mb-1">Courier</label><select value={form.courier} onChange={e => setForm({ ...form, courier: e.target.value as DeliveryTracking['courierService'] })} className="w-full px-3 py-2 border rounded-lg text-sm">{COURIER_SERVICES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
            ...
            <tbody>
              {trackings.map(t => (
                <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><select value={t.status} onChange={e => updateTrackingStatus(t.id, e.target.value as DeliveryTracking['status'])} className={`text-xs px-2 py-1 rounded-full ${getStatusColor(t.status)}`}>{['pending','picked_up','in_transit','out_for_delivery','delivered','returned'].map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}</select></td>
