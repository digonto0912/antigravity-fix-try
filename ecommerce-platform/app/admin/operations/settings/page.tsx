...
export default function SettingsPage() {
  ...
  // Shipping
  const [ship, setShip] = useState({ flatRate:'60',freeThreshold:'1000',areas:[] as string[],estDelivery:'3-5 business days' });
  // Payment
  ...
  const saveBiz = () => { updateClient({ businessName:biz.businessName,contactEmail:biz.contactEmail,contactPhone:biz.contactPhone,address:biz.address,socialMedia:{facebook:biz.facebook,instagram:biz.instagram,whatsapp:biz.whatsapp} }); addToast('Business profile saved','success'); };
  const saveStore = () => { updateClient({ storefrontSettings:{storeName:store.storeName,tagline:store.tagline,primaryColor:store.primaryColor,secondaryColor:store.secondaryColor,currency:'BDT'} }); addToast('Storefront settings saved','success'); };
  const saveShip = () => { updateClient({ shippingSettings:{flatRate:Number(ship.flatRate),freeShippingThreshold:Number(ship.freeThreshold),deliveryAreas:ship.areas,estimatedDeliveryTime:ship.estDelivery} }); addToast('Shipping saved','success'); };
  const savePay = () => { updateClient({ paymentSettings:{cod:pay.cod,bkash:{enabled:pay.bkashEnabled,merchantNumber:pay.bkashNum},nagad:{enabled:pay.nagadEnabled,merchantNumber:pay.nagadNum},bankTransfer:{enabled:pay.bankEnabled,accountDetails:pay.bankDetails}} }); addToast('Payment saved','success'); };
  const saveEmails = () => { updateClient({ emailTemplates:{orderConfirmation:{subject:emails.confSubject,body:emails.confBody},orderShipped:{subject:emails.shipSubject,body:emails.shipBody},orderDelivered:{subject:emails.delSubject,body:emails.delBody}} }); addToast('Email templates saved','success'); };
  const saveNotif = () => { updateClient({ notifications:{emailNewOrder:notif.emailNewOrder,emailLowStock:notif.emailLowStock,emailNewCustomer:notif.emailNewCustomer,emailNewMessage:notif.emailNewMessage,smsNewOrder:notif.smsNewOrder,smsLowStock:notif.smsLowStock,smsNewCustomer:notif.smsNewCustomer,smsNewMessage:notif.smsNewMessage,notificationEmail:notif.notifEmail} }); addToast('Notifications saved','success'); };

  const handleExport = () => { const data = storage.exportAll(); const blob = new Blob([data],{type:'application/json'}); const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='backup.json'; a.click(); addToast('Data exported','success'); };
  const handleImport = (e:React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if(!file) return; const reader = new FileReader(); reader.onload = (ev) => { try { storage.importAll(ev.target?.result as string); addToast('Data imported! Refresh page.','success'); } catch { addToast('Invalid file','error'); } }; reader.readAsText(file); };
  const handleClearAll = () => { if(deleteText!=='DELETE') return; storage.clearAll(); addToast('All data cleared. Refreshing...','warning'); setTimeout(()=>window.location.reload(),1500); };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border p-6 space-y-4">
        ...
        {tab==='emails'&&<><h2 className="font-semibold">Email Templates</h2>
          {[{title:'Order Confirmation',subj:emails.confSubject,body:emails.confBody,setSub:(v:string)=>setEmails({...emails,confSubject:v}),setBody:(v:string)=>setEmails({...emails,confBody:v})},{title:'Order Shipped',subj:emails.shipSubject,body:emails.shipBody,setSub:(v:string)=>setEmails({...emails,shipSubject:v}),setBody:(v:string)=>setEmails({...emails,shipBody:v})},{title:'Order Delivered',subj:emails.delSubject,body:emails.delBody,setSub:(v:string)=>setEmails({...emails,delSubject:v}),setBody:(v:string)=>setEmails({...emails,delBody:v})}].map(t=>(
