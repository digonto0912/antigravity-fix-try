...
export default function SalesAnalyticsPage() {
  ...
  const methodRevenue = orders.reduce((acc, o) => { acc[o.paymentMethod] = (acc[o.paymentMethod] || 0) + o.total; return acc; }, {} as Record<string, number>);
  const methodEntries = Object.entries(methodRevenue).sort((a, b) => b[1] - a[1]);
  const maxMethodRev = Math.max(...methodEntries.map(([, v]) => v), 1);
  const methodLabels: Record<string, string> = { cod: 'Cash on Delivery', bkash: 'bKash', nagad: 'Nagad', bank: 'Bank Transfer' };
  ...
  const statusColors: Record<string, string> = { pending: 'bg-gray-400', processing: 'bg-blue-500', shipped: 'bg-purple-500', delivered: 'bg-green-500', returned: 'bg-red-500' };
  ...
  // Revenue by city
  const cityRevenue = orders.reduce((acc, o) => { acc[o.city] = (acc[o.city] || 0) + o.total; return acc; }, {} as Record<string, number>);
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1">
            {[{ v: '7', l: '7 Days' }, { v: '30', l: '30 Days' }, { v: '90', l: '3 Months' }, { v: '365', l: 'Year' }].map(p => (
              <button key={p.v} onClick={() => setPeriod(p.v as typeof period)} className={`px-3 py-1 text-xs rounded-full ${period === p.v ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{p.l}</button>
            ))}
          </div>
        </div>
