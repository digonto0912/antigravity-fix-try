...
export default function DashboardPage() {
  ...
  const productRevenue = orders.flatMap(o => o.items).reduce((acc, item) => { acc[item.productName] = (acc[item.productName] || 0) + item.total; return acc; }, {} as Record<string, number>);
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {['pending', 'processing', 'shipped', 'delivered', 'returned'].map(status => {
              ...
              const colors: Record<string, string> = { pending: 'bg-gray-400', processing: 'bg-blue-500', shipped: 'bg-purple-500', delivered: 'bg-green-500', returned: 'bg-red-500' };
              return (
                <div key={status}>
